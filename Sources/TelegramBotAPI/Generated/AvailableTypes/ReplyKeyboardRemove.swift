...
/// hidden immediately after the user presses a button (see ``ReplyKeyboardMarkup``). Not
...
public struct ReplyKeyboardRemove: Codable, Hashable, Sendable {
    ...
    /// only. Targets: 1) users that are @mentioned in the *text* of the ``Message`` object; 2)
    /// if the bot's message is a reply to a message in the same chat and forum topic, sender of
    /// the original message. *Example:* A user votes in a poll, bot returns confirmation
    /// message in reply to the vote and removes the keyboard for that user, while still showing
